...
二进制日志是Mysql系统执行层面上的，不代表它只记录innodb日志，也包含其他的存储引擎
...
内部XA事务：确保对二进制日志的写，再对innodb的redolog的写，整个操作过程是原子的

所以binlog的刷盘分为三个阶段，其先于事务日志的刷盘：
- flush：向内存写入每个事务的二进制日志，由队列管理
- sync：将内存的二进制日志写入磁盘，由队列管理，即将队列内的日志全部刷盘
- commit：根据顺序调用存储引擎层的提交，**触发redo日志**的刷盘规则
