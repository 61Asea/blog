...
**路线：CPU寄存器 -> L1Cache/L2Cache/L3Cache -> Bus -> Memory**

### 1.1 CPU

#### 1.1.1 寄存器

寄存器是高速贮存部件，它一般用来暂存指令、数据和地址.在中央处理器的**控制部件**中，包含的寄存器有：**指令寄存器（IR）和程序计数器（PC）**；在**算术及逻辑部件**中，包含的寄存器有累加器（ACC）
...
***容量：L1 Cache < L2 Cache < L3 Cache(目前更关注，因为L1和L2基本没差别)***

***速度：L1 Cache > L2 Cache > L3Cache***

**基本单位：Cache Line缓存行，主流为64个字节大小，为缓存的最小单位**

    数据在缓存中不是以独立形式存在的，假设访问一个long数组，当数组的一个值被加载到缓存中，会额外另外7个，因此能很快速地遍历这个数组。因此，如果在数据结构中的项在内存中不是相邻的，将得不到缓存行加载带来的优势
