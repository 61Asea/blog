...
消费者的数量最好与所有主题分区的数量保持一致，多了会导致有消费者没有分区可消费，少了会导致某消费者消费多个分区

新版本通过coordinator协调者完成重平衡过程，每个新消费者在加入消费者组时都会发送JoinGroup请求给协调者，获取分区的分配策略，这个分区分配的算法逻辑由coordinator完成
