...
> `TCP窗口`就是上面在介绍重传机制中提及的缓冲区，缓冲区空间由操作系统开辟
...
- 发送窗口：主机作为数据发送方，在等到接收方的ack应答前，必须在缓冲区中保留已发送的数据，直到接收到ACK后才可以将数据从缓存区中清除

    发送窗口的大小`取决于对端通告的接收窗口大小`，一开始由**SYN报文（第一、二次握手）窗口字段中通告对方**，后续由**响应报文中的窗口字段通告**

- 接收窗口：接收方通过该**接收缓冲区**存放发送方的数据，数据将从缓存区被应用上层读取到用户进程中

    各自的接收窗口大小，取决于应用、系统、硬件的限制，TCP传输速率不能大于应用的处理速率

发送方发送的数据大小不能超过接收方的窗口大小，否则接收方就无法正常接收到数据

## **2.2 滑动窗口**

> 上面讲述SACK机制时默认使用了窗口机制

### **累计ACK**

`累计ACK`：又称为累计确认/累计应答，TCP并不是每一个报文段都会回复ACK的，可能会对两个报文段发送一个ACK，也可能会对多个报文发送一个ACK
...
因为发送窗口与对端的接收窗口一致，以单个进行分析即可：

![滑动窗口](https://asea-cch.life/upload/2021/10/%E6%BB%91%E5%8A%A8%E7%AA%97%E5%8F%A3-cdae60ab3dc246858659e2fae77afa95.png)
...
在理想情况下，当发送方一口气将数据全部发送出去后，可用窗口的大小就会变为0，以表明接收方没有处理能力了，接下来发送方在没有收到ACK确认之前是无法再继续发送数据了
