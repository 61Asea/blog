...
标准含义：每一个客观存在的现象都存在函数关系，所以让机器具备找一个函数的能力

> 假设今天想要让机器做语音识别功能，机器可以听一段声音，产生这段声音对应的文字。那这时候机器其实就具备书写一个转换函数的能力，输入是声音讯号，输出是这段声音讯号的文字内容。

## **拟合函数**

piecewise linear function：将具体函数通过微积分的方式，可以拆解成由无数个分段函数组合而成

每一个分段函数，都可以通过bias，与多个sigmoid function进行组合而成。

> sigmoid function可以由两个ReLu function进行组合而成，所以分段函数也可以由：bias + 二倍sigmoid function数量的ReLu function组合而成

hidden layer：指神经网络架构中的一层隐藏层
...
大多数的训练，在未到达critical point的时候，会先遇到loss基本不变，但gradient依旧很大的情况。

learning rate需要为每一组未知参数做定制化变化，具体通过Adagrad方法，引入西格玛参数概念，通过学习速率再去除以西格玛。

结论：西格玛 与 gradient 成正比，与 步伐 成反比

- 当loss曲线越陡峭时，每个点的gradient的绝对值就更大，这也导致了西格玛的值更大，使得学习速率除以一个更大的西格玛后产生较小的步伐
- 同理，loss曲线越平坦，则步伐则更大

Adagrad：将所有的斜率^2相加再开平方

RMSProp：引入新的参数a，使得对gradient变化更加敏感，aGt-1 + (1 - a)Gt

momentum：动量算法，引入未知参数的**上一次动量方向**，使得梯度下降法的参数移动步伐具有惯性。

Adam：RMSProp + momentum，前者使得步伐移动会随着当前斜率的大小情况（陡峭平坦情况）进行快速/减速移动，后者可以让移动具有惯性，不会随着gradient降至0值附近后骤然停止。

问题：通过Adagrad/RMSProp方法，会使得西格玛在某一段时间内被修正的很小，导致步伐突然变得大。使得参数发生井喷问题，未知参数突然变得特别的大。

通过Learning Rate Scheduling方法来降低艾塔（learning rate）的大小
- Learing Rate Decay：解决井喷问题，会跟着时间来使得艾塔（learning rate）越来越小
- Warm Up：先变大后变小，具体变大变小的速率根据实际情况调整
...
结论：机器学习，就是需要**大模型** + **大量的训练资料**。大模型指的是模型要更复杂，H的选项要足够多；大量训练资料是为了使得理想与现实更为接近。

**为什么越深越好**：对比同样参数，但是往广度发展的神经网络，深度神经网络的效果会更好。

结论：相比广度神经网络，深度神经网络是更高效的。因为深度神经网络可以通过更少的参数来达到更好的效果，降低训练资料的需求量，降低overfitting几率。

广度：需要2^k次方的参数（指数级）
深度：需要2*k个参数（常数级）

通过深度学习，可以让现实与理想更加接近，并且理想的情况会更加好。因为在深度学习中，我们可以让H变得更多但不至于过多，而且需要的训练集的要求更小。