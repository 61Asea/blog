...
- 公式思路：取key的hash值特征来得出索引，& 运算比 % 运算有更低的计算开销，但是要注意：
...
- 为什么阈值是8
    ...
    hash冲突的次数与泊松分布相关，桶内结点数量为8的概率小于百万分之一，所以桶内元素的大于等于8的概率极低

    hashmap中链表才是常态，当链表长度较小时，即使全部遍历，时间复杂度也不会很高。阈值设置为8，就是**尽量避免红黑树的出现**
