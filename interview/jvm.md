...
3. `final marking / remarking（最终标记/重标记阶段）`：出现长时间停顿的可能所在阶段
    ...
    使用增量更新机制，所以该阶段需要重新扫描根集合以保证安全，因为：
    
    - 某些赋值字节码如`astore_X`指令无法使用写屏障

    - 能使用写屏障的指令：putstatic、putfield、aastore
    
    由于收集old gen区域，且young gen没有像old gen一样通过记忆集记录，扫描时还需加入整个young gen（**已经被标记的了就无需再扫描**）
...
- **老年代可分配的剩余连续空间不够**
    ...
    > 年龄晋升、动态对象年龄判断不会直接导致full gc，而是先有young gc进行担保判断
