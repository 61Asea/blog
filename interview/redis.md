...
在4.0之后引入多线程，主要用于**持久化**、**集群同步**等异步任务

在6.0之后引入I/O线程组，线程组与socket进行绑定，对就绪socket进行网络数据**接收、解析、发送**等操作，而**命令执行操作仍旧由主线程进行**，提高cpu利用率进而提升网络I/O接收能力

# **4. 分布式锁**

使用原子操作`SET NX`实现，当key不存在时我们进行set操作，key存在时不做任何操作

> 注意：redis的set指令没有阻塞超时时间概念，所以业务上常用轮询尝试加锁的方式实现，且若有明确的同步块代码，需要先获取资源再执行业务代码，假定一定会发生锁竞争，属于悲观锁

1. expired参数有什么用？

    为必设项，用于设置key的过期时间。若没有添加expired参数，当加锁者宕机后，其他竞争者将永远无法获取到锁
...
4. 使用redis实现有什么问题呢？
    ...
    - 竞争过大：业务上竞争锁资源时可能使用尝试+轮询的方式实现，当竞争过大时应如何处理？

        解决方案：根据业务场景确定，若允许接受业务暂时挂起，则可将自旋次数和频率降低；否则，考虑将业务的**锁粒度降低**或无锁
        
        降低的方式有：锁分级管理，根据业务对共享资源的需求，将大锁划分为多个小锁

        - 竞争不激烈：转换为乐观锁，业务可采用redis的事务机制并通过multi、watch、exec等指令配合实现，或在db上

        - 竞争激烈：仍旧采用悲观锁，根据业务情况降低自旋周期
        
        > 在冲突概率大的情况下，不适合使用乐观锁，因为很大程度下都会冲突失败，而失败后又需要通过重试来保证业务正确性

    当前实现方案：锁分级管理，直接使用轮询 + 尝试的方式获取悲观锁，最大重试次数后提示操作失败
...
3. 与db的双写一致性问题
    ...
    - 写数据：先写db再删缓存、先删缓存再写db都可能出现问题，问题在于组合操作`非原子`，且可以出现`并发操作`
        ...
        > 为什么是删除缓存，而不是更新缓存：懒加载思想，假设数据为**写多读少**的场合，缓存则没必要提前计算和浪费内存去缓存该数据，而由在需要使用时再去加载

        - 非原子：第二个操作失败，不能使得第一个操作回滚，无法全部失败

        - 并发操作：该组合操作没有互斥性，不能在执行期间阻塞住其他操作，**最容易出现的就是并发查询导致的旧数据覆盖问题**

    - **双写一致**解决方案：

        - 原子性：先删cache再写db，即使写db失败了也不会造成影响，减少原子性缺失带来的问题
...
- AOF：记录的是redis命令执行操作，恢复时需要将全量日志都执行一遍
    ...
    `写后日志`的方式，即先写数据再记录日志到文件中，这是因为redis没有语法检查，先写日志可能会记录错误操作
