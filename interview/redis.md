...
1. expired参数有什么用？
    ...
    用于设置key的过期时间，且为必须设置的项目，否则当出现加锁者宕机时，锁将永远无法超时过期，与锁资源相关的其他竞争者将无法获取到锁陷入无限等待
...
4. 使用redis实现有什么问题呢？
    ...
    - 竞争过大：业务上竞争锁资源时可能使用尝试+轮询的方式实现，当竞争过大时应如何处理？
        ...
        解决方案：根据业务场景确定，若允许接受业务暂时挂起，则可将自旋次数和频率降低；否则，考虑将业务的锁粒度降低或无锁
        
        降低的方式有：锁分级管理，根据业务对共享资源的需求，将大锁划分为多个小锁

            - 竞争不激烈：转换为乐观锁，后者则可使用redis的事务机制，通过multi、watch、exec等指令配合实现
